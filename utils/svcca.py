# Copyright keep_dims18 Google Inc.
# 
...
def svcca(acts1, acts2, keep_dims=5):
  
  print(f"Results using SVCCA keeping {keep_dims} dims")
  ...
  print(f"shape of s1{s1.shape}, V1{V1.shape}")

  svacts1 = np.dot(s1[:keep_dims]*np.eye(keep_dims), V1[:keep_dims])
  # can also compute as svacts1 = np.dot(U1.T[:keep_dims], cacts1)
  svacts2 = np.dot(s2[:keep_dims]*np.eye(keep_dims), V2[:keep_dims])
  # can also compute as svacts1 = np.dot(U2.T[:keep_dims], cacts2)
  print("shape of svacts1", svacts1.shape)
  svcca_results = robust_cca_similarity(svacts1, svacts2, epsilon=1e-10)
